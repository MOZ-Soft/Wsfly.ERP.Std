...
namespace WpfRichText.Ex.AttachedProperties
{
    public static class RichTextboxAssistant
    {
        ...
        private static void HandleLostFocus(object sender, RoutedEventArgs e)
        {
            ...
            string xaml = System.Windows.Markup.XamlWriter.Save(box.Document);
            SetBoundDocument(box, xaml);
            return;
            ...
            using (MemoryStream ms = new MemoryStream())
            {
                ...
                string xamlText = ASCIIEncoding.UTF8.GetString(ms.ToArray());
        ...
        {
            var xaml = value;
            var html = HtmlFromXamlConverter.ConvertXamlToHtml(xaml, true);
